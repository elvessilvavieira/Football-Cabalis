...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <html lang="pt"><body>
    <header className="site-header"><div className="header-inner"><Link className="brand" href="/" aria-label="Cabalis Futebol — início"><span className="brand-mark"><Trophy size={20} strokeWidth={2.4} /></span><span><strong>CABALIS</strong><small>FUTEBOL</small></span></Link><nav aria-label="Navegação principal"><Link href="/">Ranking</Link><Link href="/estatisticas">Estatísticas</Link><Link href="/temporadas">Temporadas</Link></nav></div></header>
    {children}
