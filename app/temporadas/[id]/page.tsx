...
export default async function TemporadaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main>
    ...
    <section className="container content-section"><div className="section-heading"><div><span className="section-kicker"><TrendingUp size={15} /> CLASSIFICAÇÃO FINAL</span><h2>Classificação de jogadores</h2></div><p>Vitória <b>3 pts</b> · Empate <b>1 pt</b> · Derrota <b>0 pt</b></p></div><StandingsTable standings={season.standings} /><div className="team-standings-heading"><span className="section-kicker"><TrendingUp size={15} /> TIMES</span><h2>Classificação de times</h2></div><TeamStandingsTable standings={season.teamStandings} />{topScorers.length > 0 && <><div className="team-standings-heading"><span className="section-kicker"><TrendingUp size={15} /> GOLOS</span><h2>Melhores marcadores</h2></div><TopScorersTable scorers={topScorers} /></>}</section>
